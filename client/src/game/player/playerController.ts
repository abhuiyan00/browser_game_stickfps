...
export interface InputState {
  ...
  crouch?: boolean;
  /** Camera yaw in radians, rotation around the Y axis. */
  yaw: number;
}

// --- Movement tuning (Krunker-style momentum). Keep in exact sync with the
// server's copy (server/src/sim/movement.ts) or prediction fights reconciliation. ---
...
export const ARENA_HALF_EXTENT = 24; // playable square half-extent on X/Z (matches perimeter walls)
...
export function createInitialPlayerState(position: [number, number, number] = [0, GROUND_Y, 5]): PlayerState {
...
/**
 * Pure simulation step. No React/Three dependencies so it can run identically
 * client-side (prediction) and be unit-tested without a DOM or WebGL context.
 */
export function stepPlayer(state: PlayerState, input: InputState, dt: number, speedMult = 1): PlayerState {
